...
function area(data) {
    ...
    var area = d3.svg.area()
            .interpolate("step")
            .x(function (d) {
                return x(format.parse(d.TIMESTAMP));
            })
            .y0(height)
            .y1(function (d) {
                return y(d.AVERAGE_SPEED);
            });
    ...
    //Creates the small chart        
        var area2 = d3.svg.area()
            .interpolate("step")
            .x(function (d) {
                return x2(format.parse(d.TIMESTAMP));
            })
            .y0(height2)
            .y1(function (d) {
                return y2(d.AVERAGE_SPEED);
            });
    ...
    x.domain(d3.extent(data.map(function(d){return format.parse(d.TIMESTAMP)})));
    y.domain(d3.extent(data.map(function(d){return +d['AVERAGE_SPEED']})));
